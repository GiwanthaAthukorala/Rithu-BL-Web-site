...
const FacebookPixelContext = React.createContext(undefined);
function FacebookPixelProvider(props) {
    const { children, lazy = false, ...options } = props;
    const [isLoading, setIsLoading] = React.useState(true);
    const [error, setError] = React.useState();
    const pixelRef = React.useRef();
    const initPromiseRef = React.useRef();
    async function init() {
...
function FacebookProvider(props) {
    ...
    const [error, setError] = React.useState();
    const [locale, setLocaleState] = React.useState(options.language || 'en_US');
...
function useFacebook(props = {}) {
    ...
    const context = React.useContext(FacebookContext);
    if (!context) {
...
var Parser$1 = React.memo(React.forwardRef(Parser));
...
function useLogin() {
    ...
    const [latestResponse, setLatestResponse] = React.useState();
...
function useSubscribe(event, callback) {
    ...
    const { init } = useFacebook();
    const handleResponse = React.useCallback((value) => {
        setLastValue(value);
...
function useLoginStatus() {
    ...
    const [isLoading, setIsLoading] = React.useState(true);
    const [error, setError] = React.useState(undefined);
    const [status, setStatus] = React.useState(LoginStatus$1.UNKNOWN);
    const handleStatusChanges = React.useCallback((response) => {
...
exports.FacebookPixelContext = FacebookPixelContext;
